...
describe("storage", () => {
  ...
  describe("uploadImage", () => {
    it("creates directory and writes file with UUID filename", async () => {
      ...
      const result = await testStorage.uploadImage(
        fileBuffer,
        ext,
        questionId,
        eventSlug,
      ...
      expect(mockPathDirname).toHaveBeenCalledWith(
        "public/uploads/test-event/q1/test-uuid.jpg",
      );
      expect(mockMkdir).toHaveBeenCalledWith("public/uploads/test-event/q1", {
        recursive: true,
      });
    ...
    it("handles directory creation errors", async () => {
      ...
      await expect(
        testStorage.uploadImage(fileBuffer, ext, questionId, eventSlug),
  ...
  describe("cleanupImage", () => {
    it("unlinks the file from public directory", async () => {
      ...
      expect(mockPathJoin).toHaveBeenCalledWith(
        "public",
        "uploads/test-event/q1/test-uuid.jpg",
      );
