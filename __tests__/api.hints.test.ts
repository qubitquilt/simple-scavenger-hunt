...
describe("hints api", () => {
  ...
  describe("question validation", () => {
    beforeEach(() => {
      hintsPrisma.progress.findFirst.mockResolvedValue({
        id: "p1",
        eventId: "e1",
      });
    });
  ...
  describe("AI integration and response handling", () => {
    ...
    it("includes previous submissions in AI prompt", async () => {
      ...
      expect(body.messages[0].content).toContain(
        "Previous submissions and their scores",
      );
      expect(body.messages[0].content).toContain(
        'Attempt 1: ""attempt1"" - Score: 5, Status: incorrect',
      );
      expect(body.messages[0].content).toContain(
        'Attempt 2: ""attempt2"" - Score: 7, Status: incorrect',
      );
    });
