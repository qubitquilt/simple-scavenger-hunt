...
const migrateEventSlugs = async (): Promise<void> => {
  try {
    ...
    const eventsWithoutSlug = await prisma.$queryRaw<Array<{ id: string; title: string; slug: string | null }>>`
      SELECT id, title, slug FROM events WHERE slug IS NULL OR slug = '' OR slug = 'default-slug'
    `

    console.log(`Found ${eventsWithoutSlug.length} events without proper slugs`)
