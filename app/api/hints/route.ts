...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!questionData.hintEnabled) {
      return NextResponse.json({ error: 'Hints are not enabled for this question' }, { status: 403 })
    }
    ...
    for (const answer of existingHints) {
      if (answer.submission && typeof answer.submission === 'object' && 'hints' in answer.submission) {
        hintCount = Math.max(hintCount, (answer.submission as any).hints || 0)
      }
    }

    if (hintCount >= 2) {
      return NextResponse.json({ error: 'Maximum hints reached for this question' }, { status: 429 })
    }
...
Hint:`
    ...
    const submissionData = existingAnswer?.submission as Record<string, any> || {}
