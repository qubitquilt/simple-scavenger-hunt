...
function parseAllowedFormats(allowedFormats: any): ('jpg' | 'png' | 'gif')[] | null {
  ...
  try {
    ...
    if (validatedData.type === 'image') {
      if (updateDataBase.allowedFormats !== undefined) {
        updateQuestionData.allowedFormats = JSON.stringify(updateDataBase.allowedFormats)
      }
      const anyUpdateBase = updateDataBase as any
      if (anyUpdateBase.minResolution !== undefined) {
        updateQuestionData.minResolution = JSON.stringify(anyUpdateBase.minResolution)
      }
    }
