...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!file || !questionId) {
      return NextResponse.json({ error: 'File and questionId are required' }, { status: 400 })
    }
    ...
    if (questionAllowedFormats.length === 0) {
      return NextResponse.json({ error: 'No allowed formats specified' }, { status: 400 })
    }

    const questionMaxFileSize = question.maxFileSize || defaultMaxFileSize

    // Map extensions to MIME types
    const allowedMimeTypes = questionAllowedFormats.map(ext => 
      `image/${ext === 'jpg' ? 'jpeg' : ext}`
    )

    // Validate file type and size
    if (!allowedMimeTypes.includes(file.type)) {
      return NextResponse.json({ error: `Invalid file format. Allowed: ${questionAllowedFormats.join(', ')}` }, { status: 400 })
    }
    ...
    const uploadDir = path.join(process.cwd(), 'public', 'uploads', eventSlug, questionId)
