...
function AdminDashboard() {
  const [activeTab, setActiveTab] = useState<
    "metrics" | "events" | "questions" | "users"
  >("metrics");
  ...
  useEffect(() => {
    if (editingQuestion) {
      setCurrentType(
        editingQuestion.type as "text" | "multiple_choice" | "image",
      );
    } else {
      setCurrentType("text");
    }
  ...
  const handleMcQuestionSubmit = async (data: any) => {
    try {
      ...
      const body = editingQuestion ? { id: editingQuestion.id, ...data } : data;
      const res = await fetch("/api/admin/questions", {
        method,
      ...
      setError(
        `Failed to ${editingQuestion ? "update" : "create"} question: ${err.message}`,
      );
    }
  };
  ...
  if (loading && activeTab === "metrics") {
    return <div className="shadow-xl p-8">Loading...</div>;
  }
        ...
        <div className="mt-6">
          {activeTab === "metrics" && (
            <div
              id="tab-metrics"
              role="tabpanel"
              aria-labelledby="tab-metrics-button"
            >
              {metrics ? (
          ...
          {activeTab === "events" && (
            <div
              id="tab-events"
              role="tabpanel"
              aria-labelledby="tab-events-button"
              className="space-y-6"
            >
          ...
          {activeTab === "questions" && (
            <div
              id="tab-questions"
              role="tabpanel"
              aria-labelledby="tab-questions-button"
              className="space-y-6"
            >
              <div className="flex justify-between items-center">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {question.type === "multiple_choice" &&
                              question.options && (
                                <p className="text-sm">
                                  Options:{" "}
                                  {Object.keys(question.options).join(", ")}
                                </p>
                              )}
                            <p className="text-sm">
                            ...
                            <p className="text-sm">
                              AI Threshold: {question.aiThreshold}/10
                            </p>
                            <p className="text-sm">
                              Hints:{" "}
                              {question.hintEnabled ? "Enabled" : "Disabled"}
                            </p>
                          </div>
          ...
          {activeTab === "users" && (
            <div
              id="tab-users"
              role="tabpanel"
              aria-labelledby="tab-users-button"
              className="space-y-6"
            >
                      ...
                      <li key={user.id} className="px-4 py-4 sm:px-6">
                        <div className="flex items-center justify-between">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-sm">
                              Progress: {user.completedQuestions}/
                              {user.totalQuestions} (
                              {Math.round(
