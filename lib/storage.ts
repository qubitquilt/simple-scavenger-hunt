...
export interface StorageService {
  uploadImage(fileBuffer: Buffer, ext: string, questionId: string, eventSlug: string): Promise<ImageUploadResult>
  cleanupImage(url: string): Promise<void>
}

const uploadImage = async (fileBuffer: Buffer, ext: string, questionId: string, eventSlug: string): Promise<ImageUploadResult> => {
  const filename = `${randomUUID()}.${ext}`
  const filePath = path.join('public', 'uploads', eventSlug, questionId, filename)
